...
plot_titles <- list(
  ...
  subtitle = "The **Liberal Democracy Index**
    ...
    The **Electoral Democracy Index** measures the quality of elections, 
...
df_vdem_us |> 
  ...
  geom_ribbon(
    ...
    alpha = 0.2, linewidth = 0.1, col = "white", fill = "#FFFFFF"
  ) +
  geom_line(
    linewidth = 1
  ) +
  ...
  geom_label(
    ...
    hjust = 0, fill = bg_color, linewidth = 0
  ) +
  geom_textbox(
    ...
    hjust = 0, fill = alpha(bg_color, 0.2), box.size = 0, col = text_color,
  ...
  geom_label(
    ...
    family = "Instrument Sans Medium", size = 2.5,
    hjust = 0.5, fill = bg_color, linewidth = 0, col = text_color
  ) +
  geom_rect(
    data = ~filter(., year == 2017, index_label == "Liberal democracy"),
    aes(xmin = year, xmax = 2020, ymin = 0.67, ymax = 0.81),
    linewidth = 0.25, linetype = "dashed", col = text_color, fill = NA
  ) +
  ...
  theme_minimal(
    base_family = "Instrument Sans", paper = bg_color, ink = text_color) +
  theme(
...
ggsave(here("2026", "29", "29-monochrome.png"), width = 8, height = 6)
